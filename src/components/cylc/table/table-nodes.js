 ...
 * It is wrapped within a Table node (i.e. a table-node has a node), for
...
/**
 * A node of the table, with the GraphQL data objects organized
 * hierarchically.
 *
 ...
 * @property {Array<TableNode>} children - children node
 */

/**
 * @typedef {TableNode} WorkflowNode
 */
...
function createWorkflowNode (workflow) {
  // Does not have the infinite-table properties (size, state, etc) because this node is used only to build the
 ...
 *
 * @param node {GraphQLData} a table node
...
/**
 * @typedef {TableNode} CyclePointNode
 */
...
/**
 * @typedef {TableNode} FamilyProxyNode
 */
...
/**
 * @typedef {TableNode} TaskProxyNode
 */
 ...
 * @returns {boolean}
 */
function containsTableData (workflow) {
