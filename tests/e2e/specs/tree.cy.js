...
describe('Tree view', () => {
  ...
  it('Should hide jobs by default', () => {
    cy.visit('/#/workflows/one')
    cy.get('.node-data-cycle').should('be.visible')
    cy.get('.node-data-job').should('not.be.visible')
  })
  it('Should make jobs visible when clicking on tasks', () => {
    cy.visit('/#/workflows/one')
    cy.get('.node-data-job:first').should('not.be.visible')
  ...
  it('Should display leaf node triangle with margin', () => {
    ...
    cy.get('.leaf:first').should('not.be.visible')
  ...
  it('Should update view correctly', () => {
    cy.visit('/#/tree/one')
    cy.get('.node-data-cycle').should('have.length', 1)
    cy.get('.node-data-job:first').should('not.be.visible')
    cy.visit('/#/tree/anynamewillstillopenone').then(() => {
      cy.get('.node-data').should('have.length', 0)
      cy.window()
  ...
  describe('filters', () => {
    it('Should not filter by default', () => {
      cy.visit('/#/tree/one')
      cy.get('.node-data-task').contains('sleepy').should('be.visible')
      cy.get('.node-data-task').contains('waiting').should('be.visible')
    })
    it('Should filter by task name', () => {
      ...
      cy.get('#c-tree-filter-task-name').type('eep')
      cy.get('.node-data-task').contains('sleepy').should('be.visible')
      cy.get('.node-data-task').contains('waiting').should('not.be.visible')
    })
    ...
    it('Should filter by task name and states', () => {
      ...
      cy.get('#c-tree-filter-task-name').type('retry')
      cy.get('#c-tree-filter-task-states').click({ force: true })
    ...
    it('should show a summary of tasks if too many are selected', () => {
      ...
      TaskState.enumValues.forEach((state) => {
        cy.get('.v-list-item').contains(state.name).click({ force: true })
      })
      cy.get('.v-select__slot').should(($select) => {
