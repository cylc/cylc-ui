...
describe('Info component', () => {
  it('displays task information', () => {
    ...
    cy.get('.xtriggers-panel.v-expansion-panel--active').should('be.visible')
      ...
      .children()
      .then((selector) => {
        expect(selector[4].innerText).to.equal('xtrigger')
        expect(selector[5].innerText).to.equal('my-xtrigger(foo=42)')
        expect(selector[7].innerText).to.be.equal('')

        expect(selector[8].innerText).to.equal('wallclock-xtrigger')
        expect(selector[9].innerText).to.equal('wall_clock(trigger_time=634737600)')
        expect(selector[11].innerText).to.be.equal('1990-02-11T12:00:00Z')
      })
  ...
  it('should work for a task with no data', () => {
    ...
    // If there are no xtriggers, don't create the xtriggers table.
    cy.get('.xtriggers-panel')
      .eq('')
  })
